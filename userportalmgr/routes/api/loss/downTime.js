...
router.get('/', async function (req, res) {
    try {
     ...
     /*   if (tenantData.preventiveMaintenance == "plannedDowntime") {
        ...
        if (tenantData.preventiveMaintenance == "plannedDowntime") {
            console.log("preventiveMaintenance - planneddowntime");
              filter = { $and: [{ endTs: { $lte: parseInt(toTs) } }, { startTs: { $gte: parseInt(fromTs) } }], 'applId': applId, 'deviceId': deviceId, 'reason.type': { $exists: true, $nin: ["Meal Break", "Preventive Maintenance", "Planned Shutdown","Small Break"] } };
              console.log(JSON.stringify(filter))
        }
        else {
            filter = { $and: [{ endTs: { $lte: parseInt(toTs) } }, { startTs: { $gte: parseInt(fromTs) } }], 'applId': applId, 'deviceId': deviceId, 'reason.type': { $exists: true, $nin: ["Meal Break", "Planned Shutdown","Small Break"] } };
             console.log(JSON.stringify(filter))
        }
...
router.get('/slowTime', async function (req, res) {
    try {
        ...
        let project = {
            ...
            partNumber: 1, runRate:1
        };

        let sort = { date: 1 };
        let group = {
            "_id": {partNumber:"$partNumber",runRate:"$runRate"}
            ,minutes: {
                "$sum": {
                    $cond: {
                        if: {
        ...
        let project1 = {
            _id: 0, minutes: 1, count: 1,partNumber:"$_id.partNumber", runRate:"$_id.runRate"
        };

// console.log("filter : "+ JSON.stringify(filter ))
// console.log("project : "+ JSON.stringify(project ))
// console.log("group : "+ JSON.stringify(group ))
// console.log("project1 : "+ JSON.stringify(project1 ))
...
console.log("devarray: "+JSON.stringify(devArray))
        ...
        if (devArray.length > 0) {
            let slowCycleValue = 0;
            devArray.forEach((event)=>{
                console.log('### slowCycleValue 0 --> '  + slowCycleValue);
                slowCycleValue  = slowCycleValue + ( event.minutes - ( (60/event.runRate) * event.count ) );
                
            });

            
            if(slowCycleValue<0 || (!slowCycleValue))slowCycleValue=0;
            respArray.push({ "_id": "slowCycle", "timeData": Math.round(slowCycleValue) })
        }
...
router.get('/qualityLoss', async function (req, res) {
    try {
        ...
        let query = [{
            $match: { deviceId: deviceId, toTs: { $exists: true }, fromTs: { $exists: true }, $and: [{ fromTs: { $lte: parseInt(toTs) } }, { toTs: { $gte: parseInt(fromTs) } }] }
        }
            , {
            $project: { deviceId: 1, productCount: { $sum: ["$startUp", "$production"] }, timeData: { $divide: [{ $subtract: ["$toTs", "$fromTs"] }, 60000] } }
        },
        {
            $project: { deviceId: 1, quality: { $divide: [{ $subtract: [60000, '$productCount'] }, '$timeData'] } }
        }, {
            $group: { _id: '$deviceId', qualityLoss: { $sum: '$quality' } }
        }];
