...
function QualityUtil() {

    async function getQualityImpl(fromTs, toTs, deviceId) {
        let db = await DB.Get();
        let devArray = [];
    ...
    async function getQualityMetricImpl(fromTs,toTs,deviceId) {
        let devTz = 'Asia/Calcutta'
        let db = await DB.Get();
        let match = { ts: { $gte: parseInt(fromTs), $lt: parseInt(toTs) }, deviceId: deviceId, applId: "020100000104" };
        let group = {
            "_id": {
                dd: {
                    "$substr": [{
        ...
        match = { fromTs: { $gte: parseInt(fromTs) }, toTs: { $lte: parseInt(toTs) }, deviceId: deviceId };
        group = {
            "_id": {
                dd: {
                    "$substr": [{
    ...
    return {
        ...
        getQualityMetric: getQualityMetricImpl
    }
}

module.exports = QualityUtil();