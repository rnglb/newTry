...
function EnergyUtil() {
    ...
    async function getEnergyImpl(fromTs, toTs, attr, deviceId, applId, isHourly) {
        let db = await DB.Get();
        let devArray = [];
    ...
    async function getEnergySummImpl(fromTs, toTs, attr, deviceId) {
        let db = await DB.Get();
        let devArray = [];
        let filter = { toTs: { $gte: parseInt(fromTs), $lte: parseInt(toTs) }, deviceId: deviceId, totEnergy: { $gte: 0 } };
    ...
    async function newenergySummImpl(fromTs, toTs, attr, deviceId, reqtype) {
        let assetArray = [];
        let db = await DB.Get();
        let devicelist = [];
    ...
    return {
        getEnergy: getEnergyImpl,
        getEnergySumm: getEnergySummImpl,
        newEnergySumm: newenergySummImpl
    }
}
