...
let awshelper = require('../../../../shared-modules/aws-helper/middleware1.js')
  ...
  */
  router.post('/device', async function (req, res) {
    ...
    try {
      ...
      if (!newThing.certificateId) {
        ...
        console.log('newThing.certificateId:'+newThing.certificateId);
      }
  ...
  router.post('/device/activate', async function (req, res) {
    ...
    try {
     ...
     // let retrive = await Device.find({ devName: device.devName, tenantId: device.tenantId })
     let retrive={certId:'a04154715b695b06fb71b611464b7b820d36a24f92843c7cbb1166e7e9ac49ca'}
      await awshelper.Deactivatething(retrive);
   ...
   */
  router.post('/device/updatedeviceShadow', async function (req, res) {
    let input = req.body
    if (!input.tenantId) {
  ...
  cron.schedule('* * * * *', async function () {
    winston.info('running a task every minute')
    // db.otaVersion.find({otaUpdateStatus:{ $nin: ["CREATE_COMPLETE","CREATE_FAILED"]}});
    let db = await DB.Get()
    let retrive = await db
      .collection('otaVersion')
      .find({ otaUpdateStatus: { $in: ['CREATE_COMPLETE', 'CREATE_FAILED','CREATE_PENDING'] },jobId :null,jobId:{$exists: true} })
      .toArray()
  
    // change forach impl to map impl to avoid loop over response
    asyncForEach(retrive, async element => {
        console.log("element "+JSON.stringify(element));
      if (element.awsIotJobId) {
        await awshelper
          .getJobstatusCron(element.otaUpdateId)
          .then(async function (values) {
            if (values !== 'error') {
              await db
                .collection('otaVersion')
                .update(
                  { otaUpdateId: element.otaUpdateId },
                  { $set: { otaUpdateStatus: values } }
  
                )
              //
              winston.info(element.devices)
              if (element.devices) {
                for (let name in element.devices) {
                  winston.debug('name ::' + name)
                  winston.debug('element ::' + element.devices[name])
                  winston.debug('element.otaUpdateId ::' + element.otaUpdateId)
                  winston.debug('element.awsIotJobId ::' + element.awsIotJobId)
  
                  if (element.awsIotJobId) {
                    winston.debug('ota update id is not null')
  
                    await awshelper
                      .getOTAstatusCron(element.awsIotJobId, name)
                      .then(async function (values) {
                        if (values !== 'error') {
                          winston.debug("device "+name + ' ::: ' + values)
                          // update status
                          // await db.collection('otaVersion').update({ otaUpdateId: element.otaUpdateId },{ $set: {devices[name]: values} })
                        } else {
                          winston.info('values :: ::')
            ...
            /*     await awshelper
              .getOTAstatusCron(element.otaUpdateId)
              .then(async function (values) {
                if (values !== 'error') {
                  await db
                    .collection('otaVersion')
        ...
        // get and update jobid
        await awshelper
          .getJobIdArnCron(element.otaUpdateId)
          .then(async function (values) {
            if (values !== 'error') {
              // update collection
              await db.collection('otaVersion').update({ otaUpdateId: element.otaUpdateId }, { $set: { 'awsIotJobId': values.awsIotJobId, 'awsIotJobArn': values.awsIotJobArn} })
            } else {
              winston.info('values :: ::')
